...
while cap.isOpened():
    ...
    for result in results:
        ...
        for box in result.boxes:
            ...
            # Add label "Person N"
            cv2.putText(frame, f"Person {person_counter}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2, cv2.LINE_AA)
            person_counter += 1

        # Draw keypoints for each person
        keypoints = result.keypoints
        for kp in keypoints:
            # Draw keypoints for hands (left_wrist and right_wrist)
            for i, point in enumerate(kp):
                if i in [9, 10]:  # Indeks 9: left_wrist, 10: right_wrist
    ...
    if hands_results.multi_hand_landmarks:
        for hand_landmarks in hands_results.multi_hand_landmarks:
            # Get bounding box coordinates for hands
